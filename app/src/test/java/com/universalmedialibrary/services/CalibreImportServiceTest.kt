...
class CalibreImportServiceTest {
    ...
    @Test
    fun `cleanAuthorName given empty string, returns empty`() {
        val result = cleanAuthorName.invoke(calibreImportService, "") as People
        assertEquals("", result.name)
        assertEquals("", result.sortName)
    }

    @Test
    fun `cleanAuthorName given whitespace string, returns empty`() {
        val result = cleanAuthorName.invoke(calibreImportService, "   ") as People
        assertEquals("", result.name)
        assertEquals("", result.sortName)
    }
